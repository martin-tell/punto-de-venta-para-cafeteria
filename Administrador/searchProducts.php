<?php
    ...
    $consulta = mysqli_query($enlace, "select * from producto where nombre_producto like '%$busqueda%' or id_producto like '%$busqueda%' or descripcion like '%$busqueda%' or precio like '%$busqueda%' or categoria like '%$busqueda%'");
	$filas = '';
	if(mysqli_num_rows($consulta) != 0 and $busqueda != ''){
		$productos = mysqli_fetch_all($consulta, MYSQLI_ASSOC);
		foreach ($productos as $p){
            $filas .= "<tr>";                            
			$filas .= "<th scope='row'>".$p['id_producto']."</th>";
            $filas .= "<td>".$p['nombre_producto']."</td>";
            $filas .= "<td>".$p['descripcion']."</td>";
			$filas .= "<td>".$p['categoria']."</td>";
            $filas .= "<td>$".$p['precio']."</td>";
			$filas .= "<td style='display: none'>".$p['imagen']."</td>";
			$filas .= "<td class='text-center'>".($p['disponibilidad'] ? "Sí" : "No")."</td>";
