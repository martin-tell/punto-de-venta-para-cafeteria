	...
	if($filas > 0){
		...
		foreach ($rejilla as $fila){
			...
			foreach ($fila as $elemento){
				...
				$tarjetas .= "<div id='tarjeta' class='card h-100'>";
				$tarjetas .= "<img src='../imagenes_productos/" . $elemento['imagen'] . "' class='card-img-top'>";
				$tarjetas .= "<div class='card-body d-flex flex-column'>";
				$tarjetas .= "<p id='id' class='card-text' hidden>".$elemento['id_producto']."</p>";
				$tarjetas .= "<h5 id='nombre' class='card-title'>".$elemento['nombre_producto']."</h5>";
				$tarjetas .= "<p id='descripcion' class='card-text'>".$elemento['descripcion']."</p>";
				$tarjetas .= "<h5 class='mt-auto'>Precio <span id='precio' class='badge bg-secondary'>".($elemento['disponibilidad'] ? $elemento['precio'] : "No disponible")."</span></h5>";
				$tarjetas .= "</div>";
				$tarjetas .= "<div class='card-footer'>";
				$tarjetas .= "<div class='d-flex justify-content-around'>";
				$tarjetas .= "<select class='form-select w-50 w-lg-25' id='cantidad'" . ($elemento['disponibilidad'] ? "" : "disabled") . ">";
				$tarjetas .= "<option value='1'>1</option>";
				$tarjetas .= "<option value='2'>2</option>";
				$tarjetas .= "<option value='3'>3</option>";
				$tarjetas .= "<option value='4'>4</option>";
				$tarjetas .= "<option value='5'>5</option>";
				$tarjetas .= "<option value='6'>6</option>";
				$tarjetas .= "<option value='7'>7</option>";
				$tarjetas .= "<option value='8'>8</option>";
				$tarjetas .= "<option value='9'>9</option>";
				$tarjetas .= "<option value='10'>10</option>";
				$tarjetas .= "</select>";
				$tarjetas .= "<button id='btn-agregar' class='btn btn-primary'" . ($elemento['disponibilidad'] ? "" : "disabled") . ">Agregar</button>";
				$tarjetas .= "</div>";
				$tarjetas .= "</div>";
				$tarjetas .= "</div>";
				$tarjetas .= "</div>";
